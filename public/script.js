...
async function ozetBilgileriYukle() {
    try {
        ...
        const apEl = document.getElementById('musteriAktifPasifGosterge');
        if (apEl) {
            const aktif = data.aktifMusteri || 0;
            const pasif = data.pasifMusteri || 0;
            apEl.innerHTML = `<span class="text-success fw-semibold">${aktif} aktif</span> <span class="text-muted">·</span> <span class="text-secondary fw-semibold">${pasif} pasif</span> <span class="text-muted">(2025+ hareket)</span>`;
        }
...
const baslikGuncelle = (unvan) => {
    ...
    try {
        ...
        islemler.forEach(islem => {
            ...
            // Garantili Teslimat / Apartman ödeme okuma
            const aptAnlasmaSatiri = String(islem.notlar || '').includes('Apartman anlaşması');
            ...
            // Apartman: ondalık yuvarlama (0.029999... gibi saçma gösterim olmasın)
            if (aptAnlasmaSatiri) {
                kalanTeslimat = Math.round(kalanTeslimat * 100) / 100;
                teslimEdilen = Math.round(teslimEdilen * 100) / 100;
            }

            if (kalanTeslimat === 0 && !aptAnlasmaSatiri) {
                teslimDurumu = 'Teslim Edildi';
                teslimEdilen = miktar;
            } else if (kalanTeslimat === 0 && aptAnlasmaSatiri) {
                teslimEdilen = miktar;
            }
            ...
            if (borc > 0 && miktar > 0) {
                const aptFmt = (n) => (Math.round(n * 100) / 100).toLocaleString('tr-TR', { minimumFractionDigits: 0, maximumFractionDigits: 2 });
                const oLabel = aptAnlasmaSatiri ? 'Ö' : 'T';
                const kLabel = aptAnlasmaSatiri ? 'B' : 'K';
                const oTitle = aptAnlasmaSatiri ? 'Ödenen kg' : 'Teslim edilen';
                const kTitle = aptAnlasmaSatiri ? 'Kalan borç kg' : 'Kalan teslimat';
                // Kalan > 0 ise ilerleme bilgisini göster (apartmanda ödeme, normalde teslimat)
                if (kalanTeslimat > 0 || (aptAnlasmaSatiri && teslimEdilen > 0)) {
                    miktarHucresi = `
                        <div class="fw-bold fs-6">${miktar}</div>
                        <div style="font-size: 0.75rem; line-height: 1.2;" class="mt-1 text-nowrap">
                            <span class="text-success fw-bold" title="${oTitle}"><i class="fas fa-check-circle"></i> ${oLabel}: ${aptFmt(teslimEdilen)}</span> / 
                            <span class="text-danger fw-bold" title="${kTitle}"><i class="fas fa-clock"></i> ${kLabel}: ${aptFmt(kalanTeslimat)}</span>
                        </div>
                    `;
                } else {
                    // Kalan = 0 ise detayı gösterme
...
function odemeAlModalAc() {
    ...
    // Müşteri adını güvenli çöz: ünvan > resmi ad > aktif ad > başlık metni
    let gosterAd = (typeof aktifMusteriUnvan !== 'undefined' && aktifMusteriUnvan && aktifMusteriUnvan !== 'null') ? aktifMusteriUnvan : '';
    if (!gosterAd && typeof aktifMusteriAdiResmi !== 'undefined' && aktifMusteriAdiResmi) gosterAd = aktifMusteriAdiResmi;
    if (!gosterAd && aktifMusteriAd && aktifMusteriAd !== '-') gosterAd = aktifMusteriAd;
    if (!gosterAd) {
        const bul = (typeof tumMusteriler !== 'undefined' ? tumMusteriler : []).find((m) => String(m.Kimlik) === String(aktifMusteriId));
        if (bul) gosterAd = bul.Unvan || bul.Adı || bul.AdSoyad || '';
    }
    if (!gosterAd) {
        const h = document.getElementById('detayAdSoyad');
        if (h) { const t = h.innerText.replace(/Düzenle/g, '').trim(); if (t) gosterAd = t.split('\n')[0].trim(); }
    }
    if (gosterAd && (!aktifMusteriAd || aktifMusteriAd === '-')) aktifMusteriAd = gosterAd;
    document.getElementById('tahsilatMusteriAd').innerText = gosterAd || ('#' + aktifMusteriId);
    ...
    try {
        const response = await fetch('/api/musteri-odeme-makbuzlu', {
            ...
            body: JSON.stringify({
                ...
                apartmanUygula: apartmanaUygula
            })
        });
        ...
        if (response.ok && data.success) {
            ...
            } else if (apartmanaUygula && data.apartmanKg && data.apartmanKg.uyari) {
                alert('⚠️ ' + data.apartmanKg.uyari);
            } else if (apartmanaUygula && (!data.apartmanKg || !(data.apartmanKg.islenen > 0))) {
                // Apartman seçiliydi ama kg düşmedi — kullanıcıya haber ver
                console.warn('Apartman kg düşümü uygulanamadı', data.apartmanKg);
            }
...
function tabloyuGuncelle(liste) {
    ...
    gosterilecekler.forEach(musteri => {
        ...
        const durum = musteri.MusteriDurum || 'Yeni';
        const durumBadge = durum === 'Aktif'
...
function stokBirimDegisti() {
    ...
    const adetKgDiv = document.getElementById('adetKgAlanDiv');
    const alimBirimiDiv = document.getElementById('alimBirimiAlanDiv');
    ...
    if (birim === 'Çuval') {
        ...
        adetKgDiv?.classList.add('d-none');
        alimBirimiDiv?.classList.remove('d-none');
        document.getElementById('stokAdetKg').value = '';
    } else if (birim === 'Adet') {
        kgDiv.classList.add('d-none');
        adetKgDiv?.classList.remove('d-none');
        alimBirimiDiv?.classList.remove('d-none');
        document.getElementById('stokCuvalKg').value = '';
    } else {
        kgDiv.classList.add('d-none');
        adetKgDiv?.classList.add('d-none');
        alimBirimiDiv?.classList.add('d-none');
        document.getElementById('stokCuvalKg').value = '';
        document.getElementById('stokAdetKg').value = '';
    }
}
...
async function yeniStokKaydet() {
    ...
    try {
        const adetKg = birim === 'Adet' ? parseFloat(document.getElementById('stokAdetKg').value) : null;
        const alimBirimi = document.getElementById('stokAlimBirimi')?.value || null;
        const response = await fetch('/api/komur', {
            ...
            body: JSON.stringify({
                ...
                EsikAlt: altEsik,
                EsikUst: ustEsik,
                AdetBasinaKg: adetKg > 0 ? adetKg : null,
...
async function stokDuzenle(id) {
    try {
        ...
        const satisBirim = urun.birim_turu || urun.Birim || '';
        const adetKgDiv = document.getElementById('duzenleAdetKgDiv');
        const alimBirimiDiv = document.getElementById('duzenleAlimBirimiDiv');
        if (adetKgDiv) adetKgDiv.classList.toggle('d-none', satisBirim !== 'Adet');
        if (alimBirimiDiv) alimBirimiDiv.classList.toggle('d-none', satisBirim !== 'Adet' && !/çuval/i.test(satisBirim));
        const adetKgInp = document.getElementById('duzenleAdetKg');
        if (adetKgInp) adetKgInp.value = urun.adet_basina_kg || urun.AdetBasinaKg || '';
        const alimBirimiSel = document.getElementById('duzenleAlimBirimi');
        if (alimBirimiSel) alimBirimiSel.value = urun.alim_birimi || urun.AlimBirimi || 'Ton';
...
async function stokGuncelleKaydet() {
    ...
    try {
        const adetKg = parseFloat(document.getElementById('duzenleAdetKg')?.value);
        const alimBirimi = document.getElementById('duzenleAlimBirimi')?.value || null;
...
window.bekleyenTeslimatlariYukle = async function() {
    try {
        ...
        // Eski sunucu/veri kayıtlarına karşı ek güvence: apartman teslimatları
        // bu genel listede hiç gösterilmez; Apartmanlar modülünden yönetilir.
        veriler = Array.isArray(veriler) ? veriler.filter((v) => {
            if (v.IsApartmanBlok == 1) return false;
        ...
        // 🏢 APARTMAN BLOK SATIRLARI: backend'den hazır gelir (tüm daireler dahil, blok bazında)
        // Kalan = teslim edilecek kalan kömür (Anlaşılan − Teslim), borç DEĞİL.
        const normalVeriler = [];
        const blokVeriler = [];
        veriler.forEach((v) => {
            if (v.IsApartmanBlok == 1) blokVeriler.push(v);
            else normalVeriler.push(v);
        });

        blokVeriler.forEach((g) => {
            const anlasilanKg = parseFloat(g.ToplamAnlasilan) || 0;
            const teslimKg = parseFloat(g.ToplamTeslim) || 0;
            const kalanTeslimKg = Math.max(0, anlasilanKg - teslimKg);
            if (kalanTeslimKg <= 0.01) return;
            const blokAdi = (g.ApartmanBlok || '').trim();
            const mahalleGoster = g.ApartmanMahalle && g.ApartmanMahalle.trim() !== ''
                ? mahalleGorunumStr(g.ApartmanMahalle)
                : '<span class="text-danger small"><i class="fas fa-exclamation-triangle"></i> Mahalle yok</span>';
            const blokEtiket = (blokAdi && blokAdi !== '(Bloksuz)') ? `${aptKacis(blokAdi)} Blok` : 'Blok yok';
            let urun = g.UrunAdi || '-';
            if (urun.includes('(')) urun = urun.split('(')[0].trim();
            const tonGoster = (Math.round((kalanTeslimKg / 1000) * 100) / 100).toLocaleString('tr-TR', { maximumFractionDigits: 2 });
            const inenTon = (Math.round((teslimKg / 1000) * 100) / 100).toLocaleString('tr-TR', { maximumFractionDigits: 2 });
            const anlasTon = (Math.round((anlasilanKg / 1000) * 100) / 100).toLocaleString('tr-TR', { maximumFractionDigits: 2 });
            tabloGovdesi.innerHTML += `
    <tr class="sevkiyat-row border-bottom align-middle" style="background: #eef6ff; cursor:pointer;"
        title="Apartman teslimatı: Apartmanlar modülünden blok bazında girilir"
        onclick="apartmanDetayAc(${g.ApartmanId})">
        <td class="ps-4 py-2" style="width: 25%;">
            <span class="fw-bold text-dark text-uppercase"><i class="fas fa-building text-primary me-2"></i>${aptKacis(g.ApartmanAd || 'Apartman')}</span>
            <div class="small text-muted mt-1">${blokEtiket} · ${g.DaireSayisi} daire</div>
        </td>
        <td class="py-2" style="width: 15%;">
            <span class="fw-bold text-info" style="font-size: 0.9rem;"><i class="fas fa-map-marker-alt me-1"></i> ${mahalleGoster}</span>
        </td>
        <td class="py-2" style="width: 15%;">
            <span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25">${aptKacis(blokEtiket)}</span>
        </td>
        <td class="py-2" style="width: 20%;">
            <span class="fw-bold text-primary">${aptKacis(urun)}</span>
        </td>
        <td class="text-center py-2" style="width: 10%;">
            <span class="badge rounded-pill bg-primary bg-opacity-75 px-2 py-1 shadow-sm" title="Anlaşılan ${anlasTon} ton, inen ${inenTon} ton">${tonGoster} <small class="fw-normal">ton</small></span>
        </td>
        <td class="py-2" style="width: 15%;">
            <span class="text-muted small"><i class="fas fa-info-circle me-1"></i>${anlasTon}t anlaş. · ${inenTon}t inen</span>
        </td>
    </tr>
`;
        });

        veriler = normalVeriler;
...
`;
        ...
        // Sayaç: normal teslimatlar + teslimatı kalan apartman blokları
        const kalanBlokSayisi = blokVeriler.filter((g) => ((parseFloat(g.ToplamAnlasilan) || 0) - (parseFloat(g.ToplamTeslim) || 0)) > 0.01).length;
        const sayacEl = document.getElementById('toplamSevkiyatSayisi');
        if (sayacEl) sayacEl.innerText = (normalVeriler.length + kalanBlokSayisi);
    } catch (error) {
...
window.musteriTaksitleriniAc = async function() {
    ...
    document.getElementById('taksitModalBaslik').innerHTML = `
        ...
        <button class="btn btn-sm btn-danger ms-3 shadow-sm" onclick="taksitPlaniTumunuSil(${aktifMusteriId})" title="Ödenmiş taksitler korunur">
            <i class="fas fa-broom me-1"></i> Ödenmemişleri temizle
        </button>`;
...
window.borcuTaksitlendirAc = async function() { 
    ...
    try {
        ...
        if (odenmemisVarMi) {
            const onay = confirm(`⚠️ DİKKAT: MÜŞTERİNİN DEVAM EDEN TAKSİT PLANI VAR!\n\nYeni plan yaparsanız:\n\n• Ödenmemiş taksitler silinir\n• Ödenmiş / havuzda parası olan taksitler KORUNUR\n• Yeni plan eklenir\n\nOnaylıyor musunuz?`);
    ...
    const manBas = document.getElementById('yapiManuelBaslangic');
    if (manBas) manBas.value = bugun;
    const otomatikRad = document.getElementById('yapiModOtomatik');
    if (otomatikRad) otomatikRad.checked = true;
    yapiPlanModuDegisti();
    const manListe = document.getElementById('yapiManuelListe');
...
window.yapiPlanModuDegisti = function() {
    const manuel = document.getElementById('yapiModManuel')?.checked;
    document.getElementById('yapiOtomatikBlok')?.classList.toggle('d-none', !!manuel);
    document.getElementById('yapiManuelBlok')?.classList.toggle('d-none', !manuel);
    if (manuel && !document.getElementById('yapiManuelListe')?.children.length) {
        yapiManuelSatirlariOlustur();
    }
};

window.yapiManuelSatirlariOlustur = function() {
    const adet = parseInt(document.getElementById('yapiManuelSayi')?.value, 10) || 1;
    const bas = document.getElementById('yapiManuelBaslangic')?.value;
    const liste = document.getElementById('yapiManuelListe');
    if (!liste || !bas) {
        alert('Önce 1. taksit vadesini seçin.');
        return;
    }
    const bakiye = parseFloat(document.getElementById('taksitlendirilecekTutar')?.value) || 0;
    const esitTutar = adet > 0 && bakiye > 0 ? (bakiye / adet) : 0;
    let html = '<div class="row g-1 mb-1 small text-muted fw-bold"><div class="col-1">#</div><div class="col-5">Tutar (₺)</div><div class="col-6">Vade</div></div>';
    let d = new Date(bas + 'T12:00:00');
    for (let i = 1; i <= adet; i++) {
        const iso = d.toISOString().slice(0, 10);
        const vars = esitTutar > 0 ? esitTutar.toFixed(2) : '';
        html += `<div class="row g-1 mb-2 align-items-center yapi-manuel-satir">
            <div class="col-1 fw-bold small">${i}</div>
            <div class="col-5"><input type="number" class="form-control form-control-sm yapi-manuel-tutar" step="0.01" min="0.01" value="${vars}" placeholder="0,00"></div>
            <div class="col-6"><input type="date" class="form-control form-control-sm yapi-manuel-tarih" value="${iso}"></div>
        </div>`;
        d.setMonth(d.getMonth() + 1);
    }
    liste.innerHTML = html;
    liste.querySelectorAll('.yapi-manuel-tutar').forEach((inp) => inp.addEventListener('input', yapiManuelToplamGuncelle));
    yapiManuelToplamGuncelle();
};

window.yapiManuelToplamGuncelle = function() {
    const el = document.getElementById('yapiManuelToplam');
    if (!el) return;
    let t = 0;
    ...
    if (bakiye > 0 && Math.abs(t - bakiye) > 0.02) {
        ek = ` <span class="text-warning">(Bakiye: ${bakiye.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺)</span>`;
    }
    el.innerHTML = `Toplam: ${t.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺${ek}`;
};
...
window.borcYapilandirKaydet = async function() {
    ...
    if (manuel) {
        ...
        if (!satirlar.length) {
            alert('Önce taksit satırlarını oluşturun.');
            return;
        }
        const taksitler = [];
        satirlar.forEach((row) => {
            const miktar = parseFloat(row.querySelector('.yapi-manuel-tutar')?.value);
            const t = row.querySelector('.yapi-manuel-tarih')?.value;
            if (miktar > 0 && t) taksitler.push({ miktar, tarih: t });
        });
        if (taksitler.length < 1 || taksitler.length > 12) {
            alert('1 ile 12 arasında geçerli taksit girin.');
            return;
        }
        let aktifPersonel = localStorage.getItem('aktifKullanici');
        if (!aktifPersonel || aktifPersonel === 'Sistem' || aktifPersonel === 'null') {
        ...
        try {
            const res = await fetch('/api/borc-taksitlendir-manuel', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    musteri_id: aktifMusteriId,
            ...
            const data = await res.json();
            if (res.ok) {
                alert('✅ Manuel ödeme planı kaydedildi!');
                detayaGeriDon();
                musteriTaksitleriniAc();
            } else {
                alert('Hata: ' + (data.hata || 'Kayıt başarısız'));
    ...
    if (!taksit || taksit < 1 || taksit > 12) { alert("Taksit sayısı 1 ile 12 arasında olmalıdır!"); return; }
...
window.taksitPlaniTumunuSil = async function(kisiId) {
    const onay = confirm(`⚠️ ${aktifMusteriAd} müşterisinin ödenmemiş taksitleri silinecek.\n\nÖdenmiş taksitler ve havuzda parası olan kayıtlar korunur.\n\nEmin misiniz?`);
    
    if (onay) {
        try {
            const res = await fetch(`/api/taksit-plani-tumunu-sil/${kisiId}`, { method: 'DELETE' });
            if (res.ok) {
                alert("🧹 Ödenmemiş taksitler temizlendi.");
...
window.urunAlimGecmisiYukle = async function(urunId) {
    ...
    try {
        const res = await fetch(`/api/urun-alimlari/${urunId}?_t=` + Date.now());
        const yanit = await res.json();
        const alimlar = Array.isArray(yanit) ? yanit : (yanit.alimlar || []);
        const guncelKur = yanit.guncelUsdKuru || null;
        ...
        alimlar.forEach(a => {
            ...
            const h = { ...a, BirimFiyat: a.Fiyat, Borc: a.ToplamTutar };
            let ozet = alimHareketGirisOzet(h);
            if (ozet.birimMaliyetGiris <= 0 && a.Fiyat && a.Miktar) {
                const pb = a.ParaBirimi === 'USD' ? 'USD' : 'TRY';
                const stokMal = parseFloat(a.Fiyat);
                const stokM = parseFloat(a.Miktar) || 0;
                if (stokMal > 0) {
                    ozet = { ...ozet, birimMaliyetStok: stokMal, pb, pbSembol: pb === 'USD' ? '$' : '₺',
                        birimMaliyetGiris: ozet.girisMiktar > 0 && (a.DovizTutar || a.ToplamTutar)
                            ? (parseFloat(a.DovizTutar) || parseFloat(a.ToplamTutar)) / ozet.girisMiktar
                            : stokMal };
                }
            }
            const stokNot = ozet.alimBirimi !== ozet.satisBirimi
                ? `<br><small class="text-muted">${ozet.girisMiktar.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} ${ozet.alimBirimi} alım</small>`
                : '';
            const miktarTxt = `<span class="badge bg-secondary">${ozet.stokMiktar.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} ${ozet.satisBirimi}</span>${stokNot}`;
            const maliyetHtml = alimBirimFiyatHtml(ozet, guncelKur);
            tablo.innerHTML += `
                <tr class="small align-middle">
                    ...
                    <td class="text-end pe-3">${maliyetHtml}</td>
                </tr>
            `;
        });
    } catch(e) {
        console.error("Geçmiş yükleme hatası:", e);
        tablo.innerHTML = '<tr><td colspan="4" class="text-center text-danger py-3">Bağlantı hatası oluştu!</td></tr>';
    }
};

function alimDetayKutu(baslik, deger, renk) {
    const cls = renk ? ` text-${renk}` : '';
    return `<div class="col-sm-6"><div class="border rounded p-2 h-100 bg-light"><div class="text-muted small">${baslik}</div><div class="fw-bold${cls}">${deger}</div></div></div>`;
}

window.tedarikciAlimDetayAc = function(hareketId) {
    const h = (window.sonTedarikciHareketleri || []).find(x => x.ID === hareketId && x.Tur === 'ALIM');
    if (!h) { alert('Kayıt bulunamadı.'); return; }
    const ozet = alimHareketGirisOzet(h);
    const guncelKur = window.aktifTedarikciOzet?.guncelKur || null;
    ...
    document.getElementById('tedarikciAlimDetayBaslik').textContent = (h.Islem || '').replace(/^Alım:\s*/, '');

    let html = `<div class="row g-2 mb-2">${alimDetayKutu('Tarih', tarih)}${alimDetayKutu('Tedarikçi', firmaAdi)}</div>`;

    const girisTxt = `${ozet.girisMiktar.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} ${ozet.alimBirimi}`;
    const stokTxt = `${ozet.stokMiktar.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} ${ozet.satisBirimi}`;
    html += `<div class="row g-2 mb-2">${alimDetayKutu('Toptancıdan alım', girisTxt)}${alimDetayKutu('Stoğa giren', stokTxt, 'primary')}</div>`;

    const tonMaliyet = `${ozet.birimMaliyetGiris.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ${ozet.pbSembol}/${ozet.alimBirimi}`;
    let tonTl = '—';
    if (ozet.pb === 'USD' && kayitKur > 0) tonTl = `${(ozet.birimMaliyetGiris * kayitKur).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺/${ozet.alimBirimi} <small class="text-muted">(işlem kuru)</small>`;
    html += `<div class="row g-2 mb-2">${alimDetayKutu(`${ozet.alimBirimi} başına`, tonMaliyet, 'warning')}${alimDetayKutu('Defter karşılığı', tonTl)}</div>`;
    ...
    if (torbaGoster) {
        const torbaMaliyet = `${ozet.birimMaliyetStok.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ${ozet.pbSembol}/${ozet.stokBirimEtiket}`;
        let torbaTl = '—';
        if (ozet.pb === 'USD' && kayitKur > 0) torbaTl = `${(ozet.birimMaliyetStok * kayitKur).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺/${ozet.stokBirimEtiket}`;
        let guncelNot = '';
        if (ozet.pb === 'USD' && guncelKur > 0) {
            guncelNot = `<div class="col-12"><div class="alert alert-info py-2 small mb-0">Güncel kur <strong>${guncelKur.toLocaleString('tr-TR', { minimumFractionDigits: 4 })} ₺</strong> → ${ozet.alimBirimi} başına ≈ <strong>${(ozet.birimMaliyetGiris * guncelKur).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺</strong>, ${ozet.stokBirimEtiket} başına ≈ <strong>${(ozet.birimMaliyetStok * guncelKur).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺</strong></div></div>`;
        }
        html += `<div class="row g-2 mb-2">${alimDetayKutu(`${ozet.stokBirimEtiket} başına`, torbaMaliyet, 'success')}${alimDetayKutu('Defter karşılığı', torbaTl)}</div>${guncelNot}`;
    }

    let toplamHtml = '';
    if (ozet.pb === 'USD') {
        toplamHtml = `<strong>$${ozet.toplamBorcPb.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</strong> @ ${kayitKur.toLocaleString('tr-TR', { minimumFractionDigits: 4 })} → <strong>${parseFloat(h.Borc).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺</strong> deftere yazıldı`;
    } else {
        toplamHtml = `<strong>${ozet.toplamBorcPb.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺</strong> toplam borç`;
    }
    html += `<div class="border-top pt-2 mt-1"><div class="text-muted small">Toplam / defter</div><div class="fw-bold text-danger">${toplamHtml}</div></div>`;
    if (h.Aciklama && h.Aciklama !== 'null') {
        html += `<div class="mt-2 small text-muted"><em>Not: ${h.Aciklama}</em></div>`;
    }

    document.getElementById('tedarikciAlimDetayIcerik').innerHTML = html;
    const duzenleBtn = document.getElementById('tedarikciAlimDetayDuzenleBtn');
    if (duzenleBtn) {
        duzenleBtn.onclick = () => {
            modalZorlaKapat('tedarikciAlimDetayModal');
            setTimeout(() => tedarikciAlimDuzenleAc(hareketId), 300);
        };
    }
    guvenliModalAc('tedarikciAlimDetayModal');
};
...
window.stokDuzenle = async function(id) {
    try {
        ...
        const adetKgDiv = document.getElementById('duzenleAdetKgDiv');
        const alimBirimiDiv = document.getElementById('duzenleAlimBirimiDiv');
        if (adetKgDiv) adetKgDiv.classList.toggle('d-none', satisBirim !== 'Adet' && !/çuval/i.test(satisBirim));
        if (alimBirimiDiv) alimBirimiDiv.classList.toggle('d-none', satisBirim !== 'Adet' && !/çuval/i.test(satisBirim));
        const adetKgInp = document.getElementById('duzenleAdetKg');
        if (adetKgInp) {
            let kg = urun.adet_basina_kg || urun.AdetBasinaKg || '';
            if (!kg && /çuval/i.test(satisBirim)) {
                const m = String(satisBirim).match(/(\d+(?:[.,]\d+)?)\s*KG/i);
...
if (window.sistemdekiTumModallar && !window.sistemdekiTumModallar.includes('stokDonusumTopluModal')) {
    window.sistemdekiTumModallar.push('stokDonusumTopluModal');
}

function stokCuvalKgOku(birimTuru) {
    const m = String(birimTuru || '').match(/(\d+(?:[.,]\d+)?)\s*KG/i);
    return m ? parseFloat(String(m[1]).replace(',', '.')) : 0;
}

window.stokDonusumTopluModalAc = async function() {
    const tablo = document.getElementById('stokDonusumTopluTablo');
    if (!tablo) return;
    tablo.innerHTML = '<tr><td colspan="4" class="text-center py-3 text-muted">Yükleniyor...</td></tr>';
    guvenliModalAc('stokDonusumTopluModal');
    try {
        const res = await fetch('/api/komur');
        const urunler = await res.json();
        const donusumluler = urunler.filter((u) => {
            const b = u.birim_turu || u.Birim || '';
            return b === 'Adet' || /çuval/i.test(b);
        });
        if (!donusumluler.length) {
            tablo.innerHTML = '<tr><td colspan="4" class="text-center py-3 text-muted">Adet veya çuval birimli ürün yok.</td></tr>';
            return;
        }
        tablo.innerHTML = donusumluler.map((u) => {
            const birim = u.birim_turu || u.Birim || 'Adet';
            let kg = u.adet_basina_kg || u.AdetBasinaKg || '';
            if (!kg && /çuval/i.test(birim)) kg = stokCuvalKgOku(birim) || '';
            const alim = u.alim_birimi || u.AlimBirimi || 'Ton';
            const ad = (u.cins || u.UrunAdi || '').replace(/"/g, '&quot;');
            return `<tr data-id="${u.id || u.ID}">
                <td class="small fw-bold">${ad}</td>
                <td class="text-center"><span class="badge bg-secondary">${birim}</span></td>
                <td><input type="number" step="0.1" min="0" class="form-control form-control-sm toplu-kg-inp" value="${kg}" placeholder="KG"></td>
                <td><select class="form-select form-select-sm toplu-alim-inp">
        ...
        tablo.innerHTML = '<tr><td colspan="4" class="text-center text-danger py-3">Yüklenemedi</td></tr>';
    }
};

window.stokDonusumHizliKgUygula = function(tumune) {
    const kg = parseFloat(document.getElementById('topluKgHizli')?.value);
    if (!kg || kg <= 0) { alert('Önce geçerli bir KG değeri girin.'); return; }
    document.querySelectorAll('#stokDonusumTopluTablo .toplu-kg-inp').forEach((inp) => {
        if (tumune || !inp.value) inp.value = kg;
    });
};

window.stokDonusumTopluKaydet = async function() {
    const satirlar = document.querySelectorAll('#stokDonusumTopluTablo tr[data-id]');
    const kayitlar = [];
    satirlar.forEach((tr) => {
    ...
    if (!kayitlar.length) { alert('Kaydedilecek ürün yok.'); return; }
    try {
        const res = await fetch('/api/komur-donusum-toplu', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        alert(`✅ ${kayitlar.length} ürünün dönüşüm bilgisi kaydedildi.`);
        modalZorlaKapat('stokDonusumTopluModal');
        if (typeof stoklariYukle === 'function') stoklariYukle();
    } catch (err) {
        alert('Hata: ' + (err.message || 'Kayıt yapılamadı'));
    }
};
...
window.malAlimModalAc = async function() {
    ...
    const dovizGost = document.getElementById('alimDovizTutarGosterge');
    if (dovizGost) dovizGost.innerText = '—';
    ...
    if (typeof alimOdemeDurumuDegisti === 'function') alimOdemeDurumuDegisti();
    if (typeof alimKurTcmbCek === 'function') alimKurTcmbCek(true);
    ...
    try {
        ...
        urunler.forEach(u => {
            const birim = u.birim_turu || u.Birim || 'Ton';
            const adetKg = u.adet_basina_kg || u.AdetBasinaKg || '';
            const alimBirimi = u.alim_birimi || u.AlimBirimi || 'Ton';
            urunSelect.innerHTML += `<option value="${u.id || u.ID}" data-birim="${birim}" data-adet-kg="${adetKg}" data-alim-birimi="${alimBirimi}">${u.cins || u.UrunAdi}</option>`;
        });
    ...
    alimToplamEtiketGuncelle();
};
// 2. Anlık Tutar Hesaplama
window.alimOdemeDurumuDegisti = function() {
    const odeme = document.getElementById('alimOdemeDurumu')?.value || '';
    const blok = document.getElementById('alimBorcDovizBlok');
    const acikHesap = odeme.includes('Açık Hesap') || odeme.includes('Borç');
    if (blok) blok.classList.toggle('d-none', !acikHesap);
    alimToplamEtiketGuncelle();
    alimHesapla();
};

function alimSeciliUrunMeta() {
    const sel = document.getElementById('alimUrunSec');
...
function alimParseCuvalKg(birimTuru) {
    const m = String(birimTuru || '').match(/(\d+(?:[.,]\d+)?)\s*KG/i);
    return m ? parseFloat(String(m[1]).replace(',', '.')) : 0;
}

function alimGirisToStokMiktar(girisMiktar, girisBirimi, satisBirimi, adetKg) {
    ...
    const cuvalKg = adetKg > 0 ? adetKg : alimParseCuvalKg(s);
    if (g === 'Ton') {
...
window.alimUrunDegisti = function(preferredGirisBirimi) {
    const meta = alimSeciliUrunMeta();
    const gosterge = document.getElementById('alimBirimGosterge');
    const girisSel = document.getElementById('alimGirisBirimi');
    const girisKutu = document.getElementById('alimGirisBirimiKutu');
    const opts = alimGirisBirimSecenekleri(meta);
    if (girisSel) {
        girisSel.innerHTML = opts.map(b => `<option value="${b}">${b}</option>`).join('');
        const secim = preferredGirisBirimi && opts.includes(preferredGirisBirimi)
            ? preferredGirisBirimi
            : (!aktifAlimId && opts.includes('Ton') ? 'Ton' : opts[0]);
        girisSel.value = secim;
    }
    if (girisKutu) girisKutu.classList.toggle('d-none', opts.length <= 1);
    alimGirisBirimGostergeGuncelle();
    alimHesapla();
};

function alimGirisBirimSecenekleri(meta) {
    const opts = [];
    if (meta.satisBirimi === 'Adet' || /çuval/i.test(meta.satisBirimi)) {
        opts.push('Ton', 'Adet');
    } else if (meta.alimBirimi !== meta.satisBirimi) {
        opts.push(meta.alimBirimi || 'Ton', meta.satisBirimi);
    } else {
        opts.push(meta.satisBirimi || meta.alimBirimi || 'Ton');
    }
    return [...new Set(opts.filter(Boolean))];
}

function alimGirisBirimGostergeGuncelle() {
    const meta = alimSeciliUrunMeta();
    const girisSel = document.getElementById('alimGirisBirimi');
    const gosterge = document.getElementById('alimBirimGosterge');
    const birim = girisSel?.value || meta.alimBirimi || 'Ton';
    if (gosterge) gosterge.innerText = '(' + birim + ')';
}
...
function alimHareketGirisBirimi(h) {
    ...
    if (satisBirimi === 'Adet' && adetKg > 0 && stokMiktar > 0) {
        const tonMiktar = (stokMiktar * adetKg) / 1000;
        if (tonMiktar >= 0.001) return 'Ton';
    }
    if (satisBirimi === 'Adet' || /çuval/i.test(satisBirimi)) return 'Adet';
    return h.AlimBirimi || 'Ton';
}

function alimStokToGirisMiktar(stokMiktar, girisBirimi, satisBirimi, adetKg) {
    ...
    const cuvalKg = kg > 0 ? kg : alimParseCuvalKg(s);
...
function alimHareketGirisOzet(h) {
    ...
    const girisMiktar = h.GirisMiktar != null && parseFloat(h.GirisMiktar) > 0
        ...
        : alimStokToGirisMiktar(stokMiktar, alimBirimi, satisBirimi, adetKg);
...
function alimBirimFiyatHtml(ozet, guncelKur) {
    if (ozet.birimMaliyetGiris <= 0) return '<span class="text-muted">-</span>';
    let html = ozet.birimMaliyetGiris.toLocaleString('tr-TR', { minimumFractionDigits: 2 }) + ' ' + ozet.pbSembol + '/' + ozet.alimBirimi;
    const torbaGoster = (ozet.satisBirimi === 'Adet' || /çuval/i.test(ozet.satisBirimi))
        && ozet.birimMaliyetStok > 0
        && (ozet.alimBirimi !== ozet.satisBirimi || Math.abs(ozet.birimMaliyetGiris - ozet.birimMaliyetStok) > 0.005);
    if (torbaGoster) {
        html += `<br><small class="text-success fw-bold">${ozet.birimMaliyetStok.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ${ozet.pbSembol}/${ozet.stokBirimEtiket}</small>`;
    }
    const kur = parseFloat(guncelKur) || 0;
    if (ozet.pb === 'USD' && kur > 0) {
        html += `<br><small class="text-info">${(ozet.birimMaliyetGiris * kur).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺/${ozet.alimBirimi}</small>`;
        if (torbaGoster) {
            html += `<br><small class="text-info">${(ozet.birimMaliyetStok * kur).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺/${ozet.stokBirimEtiket}</small>`;
        }
    }
    return html;
}

function alimFormuHareketDoldur(h) {
    const ozet = alimHareketGirisOzet(h);
    document.getElementById('alimMiktar').value = ozet.girisMiktar > 0 ? ozet.girisMiktar : '';
    const maliyetKutu = document.getElementById('alimBirimMaliyet');
    if (maliyetKutu) maliyetKutu.value = ozet.birimMaliyetGiris > 0 ? ozet.birimMaliyetGiris.toFixed(2) : '';
    if (h.OdemeDurumu) document.getElementById('alimOdemeDurumu').value = h.OdemeDurumu;
    document.getElementById('alimAciklama').value = (h.Aciklama && h.Aciklama !== 'null') ? h.Aciklama : '';
    const pbSel = document.getElementById('alimParaBirimi');
    if (pbSel) pbSel.value = h.ParaBirimi === 'USD' ? 'USD' : 'TRY';
    if (h.ParaBirimi === 'USD' && h.IslemKuru) {
        document.getElementById('alimKur').value = parseFloat(h.IslemKuru).toFixed(4);
    }
    const kurKutu = document.getElementById('alimKurKutusu');
    if (kurKutu) kurKutu.classList.toggle('d-none', (pbSel?.value || 'TRY') === 'TRY');
    alimOdemeDurumuDegisti();
    alimToplamEtiketGuncelle();
}

function alimFormuTarihTedarikciUrunDoldur(h, tedarikciId, preferredGirisBirimi) {
    const tarihHam = h.Tarih ? String(h.Tarih).split('T')[0] : '';
    document.getElementById('alimTarih').value = tarihHam;
    const tedarikciSelect = document.getElementById('alimTedarikci');
    if (tedarikciId) {
        tedarikciSelect.value = tedarikciId;
    } else if (h.TedarikciID) {
        tedarikciSelect.value = h.TedarikciID;
    } else if (h.TedarikciFirma) {
        for (const opt of tedarikciSelect.options) {
            if (opt.text === h.TedarikciFirma) { tedarikciSelect.value = opt.value; break; }
        }
    }
    if (h.UrunID) document.getElementById('alimUrunSec').value = h.UrunID;
    alimUrunDegisti(preferredGirisBirimi || alimHareketGirisBirimi(h));
}

function alimToplamEtiketGuncelle() {
    const odeme = document.getElementById('alimOdemeDurumu')?.value || '';
    const acikHesap = odeme.includes('Açık Hesap') || odeme.includes('Borç');
    const pb = document.getElementById('alimParaBirimi')?.value || 'TRY';
    const girisBirimi = document.getElementById('alimGirisBirimi')?.value || 'Ton';
    const sembol = acikHesap && pb === 'USD' ? '$' : '₺';
    const maliyetEtiket = document.getElementById('alimBirimMaliyetEtiket');
    const toplamEtiket = document.getElementById('alimToplamEtiket');
    const toplamEtiket2 = document.getElementById('alimToplamEtiket2');
    if (maliyetEtiket) maliyetEtiket.textContent = `${girisBirimi} başına (${sembol})`;
    if (toplamEtiket) toplamEtiket.textContent = `Toplam (${sembol})`;
    if (toplamEtiket2) toplamEtiket2.textContent = acikHesap ? 'Deftere:' : 'Ödeme:';
}

window.alimGirisBirimiDegisti = function() {
    alimGirisBirimGostergeGuncelle();
    alimToplamEtiketGuncelle();
    alimHesapla();
};
...
window.alimHesapla = function() {
    const girisMiktar = parseFloat(document.getElementById('alimMiktar').value) || 0;
    const birimMaliyet = parseFloat(document.getElementById('alimBirimMaliyet')?.value) || 0;
    const pb = document.getElementById('alimParaBirimi')?.value || 'TRY';
    const kur = parseFloat(document.getElementById('alimKur')?.value) || 1;
    const odeme = document.getElementById('alimOdemeDurumu')?.value || '';
    const acikHesap = odeme.includes('Açık Hesap') || odeme.includes('Borç');
    const meta = alimSeciliUrunMeta();
    const girisBirimi = document.getElementById('alimGirisBirimi')?.value || meta.alimBirimi;
    const stokMiktar = alimGirisToStokMiktar(girisMiktar, girisBirimi, meta.satisBirimi, meta.adetKg);
    ...
    const pbSembol = acikHesap && pb === 'USD' ? '$' : '₺';

    const toplamHidden = document.getElementById('alimToplamBorc');
    if (toplamHidden) toplamHidden.value = toplamBorc > 0 ? toplamBorc.toFixed(2) : '';
    const toplamGost = document.getElementById('alimToplamGosterge');
    if (toplamGost) toplamGost.textContent = toplamBorc > 0
        ? toplamBorc.toLocaleString('tr-TR', { minimumFractionDigits: 2 }) + ' ' + pbSembol
        : '0,00';

    const birimFiyatHidden = document.getElementById('alimBirimFiyat');
    if (birimFiyatHidden) birimFiyatHidden.value = birimFiyatStok > 0 ? birimFiyatStok.toFixed(4) : '';

    const birimFiyatGost = document.getElementById('alimBirimFiyatHesap');
    if (birimFiyatGost) {
        birimFiyatGost.textContent = stokMiktar > 0 && toplamBorc > 0
    ...
    const donusumNot = document.getElementById('alimDonusumNotu');
    const stokGost = document.getElementById('alimStokMiktarGosterge');
    if (girisBirimi !== meta.satisBirimi && stokMiktar > 0 && girisMiktar > 0) {
        const not = `→ Stoğa ${stokMiktar.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} ${meta.satisBirimi}`;
        if (donusumNot) donusumNot.textContent = not;
        if (stokGost) stokGost.textContent = not;
    } else {
        if (donusumNot) donusumNot.textContent = '';
        if (stokGost) stokGost.textContent = stokMiktar > 0 ? `Stok: ${stokMiktar.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} ${meta.satisBirimi}` : '';
    }

    let tlToplam = toplamBorc;
    const dovizGost = document.getElementById('alimDovizTutarGosterge');
    const ozetEl = document.getElementById('alimHesapOzeti');

    if (acikHesap && pb === 'USD') {
        tlToplam = toplamBorc * kur;
        if (dovizGost && toplamBorc > 0) {
            dovizGost.innerHTML = `<strong>$${toplamBorc.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</strong> borç`;
        } else if (dovizGost) dovizGost.textContent = '—';
        if (ozetEl && toplamBorc > 0) {
            ozetEl.innerHTML = `× kur ${kur.toLocaleString('tr-TR', { minimumFractionDigits: 4 })} → <strong>${tlToplam.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺</strong>`;
        } else if (ozetEl) ozetEl.textContent = '';
    } else {
        if (dovizGost) dovizGost.textContent = '—';
        if (ozetEl) ozetEl.textContent = '';
    }
    document.getElementById('alimToplamTutarGosterge').innerText = tlToplam.toLocaleString('tr-TR', { minimumFractionDigits: 2 }) + ' ₺';
};

window.alimParaBirimiDegisti = function() {
    const pb = document.getElementById('alimParaBirimi')?.value || 'TRY';
    const kurKutu = document.getElementById('alimKurKutusu');
    if (kurKutu) kurKutu.classList.toggle('d-none', pb === 'TRY');
    alimToplamEtiketGuncelle();
    if (pb === 'USD') alimKurTcmbCek(true);
    alimHesapla();
};

window.alimKurTcmbCek = async function(sessiz) {
    try {
        const res = await fetch('/api/tcmb-piyasa');
        const data = await res.json();
        if (data.ok && data.usd?.satis) {
            const kurStr = String(data.usd.satis).replace(/\./g, '').replace(',', '.');
            const kur = parseFloat(kurStr);
            if (kur > 0) {
                document.getElementById('alimKur').value = kur.toFixed(4);
                const not = document.getElementById('alimGuncelKurNotu');
                if (not) not.textContent = `TCMB: ${data.usd.satis} ₺`;
                alimHesapla();
                return;
            }
        }
        if (!sessiz) alert('TCMB kuru alınamadı, elle girin.');
    } catch (_e) {
        if (!sessiz) alert('Kur servisi yanıt vermedi.');
    }
};
...
window.malAlimiKaydet = async function() {
    ...
    const girisMiktar = parseFloat(document.getElementById('alimMiktar').value);
    const birimMaliyet = parseFloat(document.getElementById('alimBirimMaliyet')?.value) || 0;
    ...
    const acikHesap = odeme.includes('Açık Hesap') || odeme.includes('Borç');
    const paraBirimi = acikHesap ? (document.getElementById('alimParaBirimi')?.value || 'TRY') : 'TRY';
    const islemKuru = paraBirimi === 'USD' ? (parseFloat(document.getElementById('alimKur')?.value) || 0) : 1;

    if(!tarih || !tedarikciId || !urunId || girisMiktar <= 0 || stokMiktar <= 0) {
        alert("Lütfen miktar alanını doldurun!"); return;
    }
    if (toplamBorcKayit <= 0) {
        alert("Miktar ve birim maliyet girin!"); return;
    }
    ...
    if (girisBirimi !== meta.satisBirimi && meta.satisBirimi === 'Adet' && meta.adetKg <= 0) {
        alert("Bu ürün için adet başına KG tanımlı değil. Ürün düzenlemeden '1 Adet Kaç KG' girin."); return;
    }
    if (acikHesap && paraBirimi === 'USD' && islemKuru <= 0) {
        alert("USD borç için geçerli bir kur girin!"); return;
    }
...
window.alimGecmisiYukle = async function() {
    ...
    try {
        ...
        alimlar.forEach(a => {
            ...
            const ozet = alimHareketGirisOzet(a);
            const miktarTxt = ozet.girisMiktar > 0
                ? `${ozet.girisMiktar.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} ${ozet.alimBirimi}`
                : parseFloat(a.Miktar).toLocaleString('tr-TR');
            const birimTxt = ozet.birimMaliyetGiris > 0
                ? alimBirimFiyatHtml(ozet)
                : parseFloat(a.BirimMaliyet).toLocaleString('tr-TR', { minimumFractionDigits: 2 }) + ' ' + (a.ParaBirimi === 'USD' ? '$' : '₺');
            ...
            const kurNot = (a.ParaBirimi === 'USD' && a.DovizTutar)
                ? `<br><small class="text-info">$${parseFloat(a.DovizTutar).toLocaleString('tr-TR', {minimumFractionDigits:2})} @ ${parseFloat(a.IslemKuru).toLocaleString('tr-TR', {minimumFractionDigits:4})}</small>`
                : '';
            ...
            tablo.innerHTML += `
                <tr>
                    ...
                    <td class="text-center fw-bold fs-6">${miktarTxt}</td>
                    <td class="text-end text-danger">${birimTxt}</td>
                    <td class="text-end fw-bold text-dark">${toplam} ₺${kurNot}</td>
                    <td class="text-center">${odemeBadge}</td>
                    <td class="text-center text-nowrap">
                        <button class="btn btn-sm btn-outline-warning shadow-sm me-1" 
                                onclick="alimDuzenleAc(${a.ID})" title="Düzenle">
                            <i class="fas fa-edit"></i>
                        </button>
...
window.alimDuzenleAc = async function(id) {
    const h = (window.sonAlimGecmisi || []).find(a => a.ID === id);
    if (!h) { alert('Kayıt bulunamadı. Listeyi yenileyin.'); return; }
    await malAlimModalAc();
    aktifAlimId = id;
    alimFormuTarihTedarikciUrunDoldur(h);
    alimFormuHareketDoldur(h);
    alimHesapla();
};

window.tedarikciAlimDuzenleAc = async function(hareketId) {
    const h = (window.sonTedarikciHareketleri || []).find(x => x.ID === hareketId && x.Tur === 'ALIM');
    if (!h) { alert('Kayıt bulunamadı.'); return; }
    modalZorlaKapat('tedarikciDetayModal');
    setTimeout(async () => {
        await malAlimModalAc();
        aktifAlimId = hareketId;
        alimFormuTarihTedarikciUrunDoldur(h, aktifTedarikciId);
        alimFormuHareketDoldur(h);
        alimHesapla();
    }, 300);
};
...
window.tedarikcileriYukle = async function() {
    ...
    try {
        ...
        veriler.forEach(t => {
            ...
            const guncelKur = t.GuncelUsdKuru;
            const guncelTl = t.GuncelTlKarsiligi;
            ...
            let kurNot = '';
            if (dovizBorc > 0 && guncelKur) {
                kurNot = `<br><small class="text-info">$${dovizBorc.toLocaleString('tr-TR', {minimumFractionDigits:2})} · güncel kur ${guncelKur.toLocaleString('tr-TR', {minimumFractionDigits:4})} → ${(guncelTl || 0).toLocaleString('tr-TR', {minimumFractionDigits:2})} ₺</small>`;
            } else if (bakiye > 0 && guncelKur) {
                kurNot = `<br><small class="text-muted">Güncel kur notu: ${bakiye.toLocaleString('tr-TR', {minimumFractionDigits:2})} ₺</small>`;
            }
            ...
            tablo.innerHTML += `
                <tr>
                    ...
                    <td class="text-end fw-bold ${bakiyeRenk}">
                        ...
                        <small class="text-muted">${bakiyeYazi}</small>${kurNot}
                    </td>
                    <td class="text-center">
...
window.tedarikciDetayYukle = async function(id, firmaAdi) {
    ...
    try {
        ...
        window.sonTedarikciHareketleri = hareketler;
        const guncelKur = Array.isArray(yanit) ? null : yanit.guncelUsdKuru;

        tablo.innerHTML = '';
        const kurNotBlok = document.getElementById('tedarikciKurNotu');
        const kurNotMetin = document.getElementById('tedarikciKurNotuMetin');
        if(hareketler.length === 0) {
            ...
            if (kurNotBlok) kurNotBlok.classList.add('d-none');
            return;
        }
        ...
        hareketler.forEach(h => {
            ...
            let miktarTxt = '<span class="text-muted">-</span>';
            let birimTxt = '<span class="text-muted">-</span>';
            if (h.Tur === 'ALIM' && h.Miktar) {
                const ozet = alimHareketGirisOzet(h);
                const stokNot = ozet.alimBirimi !== ozet.satisBirimi && ozet.stokMiktar > 0
                    ? `<br><small class="text-muted">→ ${ozet.stokMiktar.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} ${ozet.satisBirimi}</small>`
                    : '';
                miktarTxt = `<span class="badge bg-secondary">${ozet.girisMiktar.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} ${ozet.alimBirimi}</span>${stokNot}`;
                birimTxt = alimBirimFiyatHtml(ozet, guncelKur);
            }
            ...
            const odemeKurEk = (h.Tur === 'ODEME' && h.ParaBirimi === 'USD' && h.DovizTutar)
                ? `<br><small class="text-info">$${parseFloat(h.DovizTutar).toLocaleString('tr-TR', {minimumFractionDigits:2})} @ ${parseFloat(h.IslemKuru || 1).toLocaleString('tr-TR', {minimumFractionDigits:4})}</small>`
                : '';
            const kurEk = (h.ParaBirimi === 'USD' && h.DovizTutar)
                ? `<br><small class="text-info">$${parseFloat(h.DovizTutar).toLocaleString('tr-TR', {minimumFractionDigits:2})} @ ${parseFloat(h.IslemKuru || 1).toLocaleString('tr-TR', {minimumFractionDigits:4})}</small>`
                : '';
            
            const islemId = h.ID; 
            const islemTur = h.Tur;
            
            const infoBtn = islemTur === 'ALIM'
                ? `<button class="btn btn-sm btn-outline-info shadow-sm" onclick="tedarikciAlimDetayAc(${islemId})" title="Alım maliyet detayı"><i class="fas fa-info-circle"></i></button>`
                : '';
            const duzenleBtn = islemTur === 'ALIM'
                ? `<button class="btn btn-sm btn-outline-warning shadow-sm" onclick="tedarikciAlimDuzenleAc(${islemId})" title="Düzenle"><i class="fas fa-edit"></i></button>`
                : '';
            const silButon = `<button class="btn btn-sm btn-outline-danger shadow-sm" onclick="tedarikciHareketSil(${islemId}, '${islemTur}')" title="Sil"><i class="fas fa-trash-alt"></i></button>`;
            const islemBtns = `<div class="btn-group btn-group-sm">${infoBtn}${duzenleBtn}${silButon}</div>`;
            ...
            tablo.innerHTML += `
                <tr class="align-middle small">
                    ...
                    <td class="text-end text-danger fw-bold">${borcTxt}${kurEk}</td>
                    <td class="text-end text-success fw-bold">${odemeTxt}${odemeKurEk}</td>
                    <td class="text-muted">${h.Aciklama || '-'}</td>
                    <td class="text-center text-nowrap">${islemBtns}</td>
                </tr>
            `;
        });

        window.aktifTedarikciOzet = { tlBakiye: yuruyenBakiye, usdBorc: Math.max(0, toplamDovizBorc), guncelKur: guncelKur || null };
        ...
        document.getElementById('ozetGuncelBakiye').className = yuruyenBakiye > 0 ? "h5 mb-0 text-danger fw-bold" : "h5 mb-0 text-success fw-bold";

        if (kurNotBlok && kurNotMetin) {
            if (yuruyenBakiye > 0 && guncelKur) {
                const guncelTl = toplamDovizBorc > 0
                    ? toplamDovizBorc * guncelKur
                    : yuruyenBakiye;
                let metin = `Güncel TCMB satış kuru: <strong>${guncelKur.toLocaleString('tr-TR', {minimumFractionDigits:4})} ₺</strong> — `;
                if (toplamDovizBorc > 0) {
                    metin += `Kayıtlı döviz borcu <strong>$${toplamDovizBorc.toLocaleString('tr-TR', {minimumFractionDigits:2})}</strong> güncel kura göre <strong>${guncelTl.toLocaleString('tr-TR', {minimumFractionDigits:2})} ₺</strong> karşılığındadır. Defter bakiyesi: <strong>${yuruyenBakiye.toLocaleString('tr-TR', {minimumFractionDigits:2})} ₺</strong>`;
                } else {
                    metin += `Defter borcu güncel kura göre <strong>${yuruyenBakiye.toLocaleString('tr-TR', {minimumFractionDigits:2})} ₺</strong>`;
                }
                kurNotMetin.innerHTML = metin;
                kurNotBlok.classList.remove('d-none');
            } else {
                kurNotBlok.classList.add('d-none');
            }
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (urunSecKutusu) {
        urunSecKutusu.addEventListener('change', alimUrunDegisti);
    }
...
window.tedarikciOdemeModalAc = function() {
    ...
    document.getElementById('odemeTutari').value = '';
    document.getElementById('odemeAciklama').value = '';
    document.getElementById('odemeParaBirimi').value = 'TRY';
    const ozet = window.aktifTedarikciOzet || {};
    const borcEl = document.getElementById('odemeBorcOzet');
    if (borcEl) {
        let metin = `Defter borcu: <strong>${(ozet.tlBakiye || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺</strong>`;
        if (ozet.usdBorc > 0) {
            metin += ` · Döviz borcu: <strong>$${ozet.usdBorc.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</strong>`;
            if (ozet.guncelKur) metin += ` (≈ ${(ozet.usdBorc * ozet.guncelKur).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺)`;
        }
        borcEl.innerHTML = metin;
    }
    odemeParaBirimiDegisti();
    if (ozet.guncelKur) document.getElementById('odemeKur').value = parseFloat(ozet.guncelKur).toFixed(4);
    guvenliModalAc('tedarikciOdemeModal');
};

window.odemeParaBirimiDegisti = function() {
    const pb = document.getElementById('odemeParaBirimi')?.value || 'TRY';
    const kurKutu = document.getElementById('odemeKurKutusu');
    const etiket = document.getElementById('odemeTutarEtiket');
    if (kurKutu) kurKutu.classList.toggle('d-none', pb === 'TRY');
    if (etiket) etiket.textContent = pb === 'USD' ? 'Ödenen Tutar ($)' : 'Ödenen Tutar (₺)';
    if (pb === 'USD') odemeKurTcmbCek(true);
    odemeTutarHesapla();
};

window.odemeTutarHesapla = function() {
    const pb = document.getElementById('odemeParaBirimi')?.value || 'TRY';
    const tutar = parseFloat(document.getElementById('odemeTutari')?.value) || 0;
    const kur = parseFloat(document.getElementById('odemeKur')?.value) || 0;
    const defterEl = document.getElementById('odemeDefterTutar');
    if (!defterEl) return;
    if (pb === 'USD' && tutar > 0 && kur > 0) {
        defterEl.textContent = (tutar * kur).toLocaleString('tr-TR', { minimumFractionDigits: 2 }) + ' ₺';
    } else if (pb === 'TRY' && tutar > 0) {
        defterEl.textContent = tutar.toLocaleString('tr-TR', { minimumFractionDigits: 2 }) + ' ₺';
    } else {
        defterEl.textContent = '0,00 ₺';
    }
};

window.odemeKurTcmbCek = async function(sessiz) {
    try {
        const res = await fetch('/api/tcmb-piyasa');
        const data = await res.json();
        if (data.ok && data.usd?.satis) {
            ...
            const kur = parseFloat(kurStr);
            if (kur > 0) {
                document.getElementById('odemeKur').value = kur.toFixed(4);
                odemeTutarHesapla();
                return;
            }
        }
        if (!sessiz) alert('Kur alınamadı.');
    } catch (_e) { if (!sessiz) alert('Kur alınamadı.'); }
};
...
window.tedarikciOdemeKaydet = async function() {
    ...
    const paraBirimi = document.getElementById('odemeParaBirimi')?.value || 'TRY';
    const islemKuru = paraBirimi === 'USD' ? (parseFloat(document.getElementById('odemeKur')?.value) || 0) : 1;
    ...
    if (paraBirimi === 'USD' && islemKuru <= 0) {
        alert("USD ödeme için geçerli bir kur girin!");
        return;
    }
    ...
    try {
        const res = await fetch('/api/tedarikci-odeme', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ tedarikciId: aktifTedarikciId, tarih, tutar, tur, aciklama, paraBirimi, islemKuru }) 
        });

        if(res.ok) {
            const pb = paraBirimi === 'USD' ? '$' : '₺';
            const msg = paraBirimi === 'USD'
                ? `✅ $${tutar.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ödeme kaydedildi (defter: ${(tutar * islemKuru).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺)`
                : `✅ ${tutar.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺ ödeme kaydedildi.`;
            alert(msg);
            odemeEkranindanGeriDon(); 
        } else {
            const err = await res.json().catch(() => ({}));
            alert("Hata: " + (err.hata || "Kaydedilemedi."));
        }
    } catch (err) { alert("Bağlantı hatası!"); }
};
...
// Bir sevkiyat satırını (normal veya apartman blok) rapor için tek biçime indirger
window.raporSatirNormalize = function(islem) {
    // 🏢 Apartman blok satırı
    if (islem && islem.IsApartmanBlok) {
        const anlas = parseFloat(islem.ToplamAnlasilan) || 0;
        const teslim = parseFloat(islem.ToplamTeslim) || 0;
        const kalanKg = Math.max(0, Math.round((anlas - teslim) * 100) / 100);
        const hamMah = (islem.ApartmanMahalle && islem.ApartmanMahalle !== 'null' && String(islem.ApartmanMahalle).trim() !== '')
            ? String(islem.ApartmanMahalle).trim() : '';
        return {
            apartman: true,
            mahalleHam: hamMah,
            unvan: `${islem.ApartmanAd || ''} · ${islem.ApartmanBlok || ''} Blok`.trim(),
            urun: (islem.UrunAdi && islem.UrunAdi !== 'null') ? islem.UrunAdi : 'Kömür',
            miktar: kalanKg,
            birim: 'kg',
            tel: 'Apartman',
            adres: [islem.ApartmanIlce, hamMah].filter((x) => x && x !== 'null').join(' / ') || '-',
            notlar: `${islem.DaireSayisi || 0} daire`
        };
    }
    // Normal müşteri hareketi satırı
    const hamMah = (islem.Mahalle && islem.Mahalle !== 'null' && islem.Mahalle.trim() !== '')
        ? islem.Mahalle.trim() : '';
    let tamAciklama = islem.AÇIKLAMA || '';
    let urunAdiTemiz = tamAciklama;
    if (tamAciklama.includes(' x ')) urunAdiTemiz = tamAciklama.split(' x ')[1].split(' (')[0];
    ...
    if (!dinamikBirim || dinamikBirim === 'Birim' || dinamikBirim === '-') {
        const l = urunAdiTemiz.toLowerCase();
        dinamikBirim = (l.includes('un') || l.includes('yem') || l.includes('çuval') || l.includes('torba')) ? 'Çuval' : 'Adet';
    }
    let ad = (islem.Adı && islem.Adı !== 'null') ? islem.Adı : '';
    let soyad = (islem.Soyadı && islem.Soyadı !== 'null') ? islem.Soyadı : '';
    let unvan = (islem.Unvan && islem.Unvan !== 'null' && islem.Unvan.trim() !== '') ? islem.Unvan : `${ad} ${soyad}`.trim();
    ...
    return {
        ...
        unvan,
        urun: urunAdiTemiz,
        miktar: parseInt(islem.KalanTeslimat || islem.ADET || 0),
        birim: dinamikBirim,
        tel,
        adres,
        notlar: islem.notlar ? islem.notlar : ''
    };
};
...
window.raporMerkeziAc = async function() {
    try {
        ...
        // Mahalleleri çıkar ve Select'i doldur (apartmanlar dahil)
        const mahalleler = new Set();
        veriler.forEach(i => {
            const n = raporSatirNormalize(i);
            const m = n.mahalleHam ? mahalleGorunumStr(n.mahalleHam) : mahalleRaporEksikEtiketi();
            mahalleler.add(m);
        });
...
window.raporOnizlemeGuncelle = function() {
    ...
    veriler.forEach(islem => {
        const n = raporSatirNormalize(islem);
        const m = n.mahalleHam ? mahalleGorunumStr(n.mahalleHam) : mahalleRaporEksikEtiketi();
        if (seciliMahalle !== 'TUMU' && m !== seciliMahalle) return;

        gosterilenSayi++;

        // 🚨 1. SOL TARAF İÇİN: Ürün adına göre topla (birim ayrı ayrı toplanır)
        let urunAnahtari = (n.urun || '').toUpperCase() + ' • ' + n.birim;
        if (!urunToplamlari[urunAnahtari]) {
            urunToplamlari[urunAnahtari] = { ad: (n.urun || '').toUpperCase(), miktar: 0, birim: n.birim };
        }
        urunToplamlari[urunAnahtari].miktar += n.miktar;

        // 🚨 2. SAĞ TARAF İÇİN: Detay satırını oluştur
        const satirStil = n.apartman ? ' style="background:#eef7ff;"' : '';
        const rozet = n.apartman ? ' <span class="badge bg-info text-dark" style="font-size:0.6rem;">APT</span>' : '';
        detayHtml += `
            <tr${satirStil}>
                <td class="fw-bold text-dark" style="font-size: 0.9rem;">${n.unvan.toUpperCase()}${rozet}</td>
                <td class="text-primary" style="font-size: 0.85rem;">${n.urun}</td>
                <td class="text-center fw-bold text-danger" style="font-size: 0.9rem;">
                    ${n.miktar} <span style="font-size: 0.75rem; font-weight: normal; color: #666;">${n.birim}</span>
                </td>
            </tr>
        `;
    });
...
window.seciliRaporuYazdir = function() {
    ...
    veriler.forEach(islem => {
        const n = raporSatirNormalize(islem);
        const mahalle = n.mahalleHam ? mahalleGorunumStr(n.mahalleHam) : mahalleRaporEksikEtiketi();
        ...
        if (!gruplar[mahalle].urunToplamlari[urunAnahtari]) {
            gruplar[mahalle].urunToplamlari[urunAnahtari] = { ad: (n.urun || '').toUpperCase(), miktar: 0, birim: n.birim };
        }
        gruplar[mahalle].urunToplamlari[urunAnahtari].miktar += n.miktar;
    });
    ...
    for (const mahalle in gruplar) {
        ...
        for (const data of Object.values(gruplar[mahalle].urunToplamlari)) {
            html += `<span class="ozet-badge">${data.ad}: ${data.miktar} ${data.birim}</span>`;
        }
        html += `</div>`;
        ...
        gruplar[mahalle].islemler.forEach(islem => {
            const n = raporSatirNormalize(islem);
            const rozet = n.apartman ? ' [APARTMAN]' : '';
            html += `
                <tr>
                    <td><strong>${n.unvan.toUpperCase()}${rozet}</strong></td>
                    <td>${n.tel}</td>
                    <td>${n.urun}</td>
                    <td class="text-center kalan">${n.miktar} <span style="font-size:10px; color:#555;">${n.birim}</span></td>
                    <td>
                        <div>${n.adres}</div>
                        ${n.notlar ? `<div class="not-kutu"><b>Not:</b> ${n.notlar}</div>` : ''}
                    </td>
                </tr>
            `;
        });
...
function aptPara(n) {
    return (parseFloat(n) || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2 }) + ' ₺';
}
...
/** Daire ödenen kg: önce KalanBorcKg farkı; yoksa OdenenTl'den kabaca tahmin */
function aptOdenenKg(d) {
    const anlasilan = parseFloat(d.AnlasilanMiktar) || 0;
    const borcKg = d.KalanBorcKg != null ? parseFloat(d.KalanBorcKg) : anlasilan;
    let odenen = Math.max(0, Math.round((anlasilan - borcKg) * 100) / 100);
    if (odenen < 0.01) {
        const odTl = parseFloat(d.OdenenTl) || 0;
        if (odTl > 0.01) {
            const para = String(d.ParaBirimi || 'TRY').toUpperCase();
            const ton = parseFloat(d.TonFiyat) || ((parseFloat(d.BirimFiyat) || 0) * 1000);
            const kgFiyat = parseFloat(d.BirimFiyat) || (ton > 0 ? ton / 1000 : 0);
            const kur = para === 'USD' ? (parseFloat(d.AnlasmaKuru) || 0) : 1;
            const kgBirimTl = kgFiyat * (kur > 0 ? kur : 1);
            if (kgBirimTl > 0) {
                odenen = Math.min(anlasilan, Math.round((odTl / kgBirimTl) * 100) / 100);
            }
        }
    }
    return odenen;
...
function aptBorcKg(d) {
    ...
    // OdenenTl'den tahmin varsa borç = anlaşılan - ödenen
...
function aptBlokListesi(daireler) {
    const set = new Set();
    (daireler || []).forEach((d) => set.add((d.Blok && String(d.Blok).trim()) || '(Bloksuz)'));
    return Array.from(set).sort((a, b) => a.localeCompare(b, 'tr'));
}

function aptBlokFiltreliDaireler(daireler, blok) {
    if (!blok || blok === 'TUMU') return daireler || [];
    return (daireler || []).filter((d) => {
        const b = (d.Blok && String(d.Blok).trim()) || '(Bloksuz)';
        return b === blok;
    });
}

function aptTonFiyatKgGoster(tonFiyat, para) {
    const t = parseFloat(tonFiyat) || 0;
    if (!t) return '—';
    const kg = t / 1000;
    const pb = para === 'USD' ? 'USD' : '₺';
    return `${aptSayi(kg)} ${pb}/kg`;
}

function aptBlokSekmeleriCiz(containerId, bloklar, aktif, onClickFn) {
    const el = document.getElementById(containerId);
    if (!el) return;
    const tum = ['TUMU', ...bloklar.filter((b) => b !== '(Bloksuz)')];
    if (bloklar.includes('(Bloksuz)')) tum.push('(Bloksuz)');
    el.innerHTML = tum.map((b) => {
        const lbl = b === 'TUMU' ? 'Tümü' : b;
        const cls = b === aktif ? 'active' : '';
        return `<li class="nav-item"><button type="button" class="nav-link ${cls}" onclick="${onClickFn}('${aptKacis(b).replace(/'/g, "\\'")}')">${aptKacis(lbl)}</button></li>`;
    }).join('');
}

function daireBlokNoSecenekleriYukle(seciliBlok, seciliNo) {
    const daireler = window.aktifApartmanDaireler || [];
    const bloklar = aptBlokListesi(daireler);
    const blokSel = document.getElementById('dDuzBlokSec');
    const noSel = document.getElementById('dDuzNoSec');
    if (!blokSel || !noSel) return;
    blokSel.innerHTML = bloklar.map((b) => `<option value="${aptKacis(b)}">${aptKacis(b === '(Bloksuz)' ? '— Bloksuz —' : b)}</option>`).join('');
    const aktifBlok = seciliBlok || bloklar[0] || '';
    blokSel.value = aktifBlok;
    const noList = daireler.filter((d) => ((d.Blok && String(d.Blok).trim()) || '(Bloksuz)') === aktifBlok);
    noSel.innerHTML = noList.map((d) => `<option value="${aptKacis(d.DaireNo || '')}">${aptKacis(d.DaireNo || '?')}${d.DaireTipi ? ' · ' + aptKacis(d.DaireTipi) : ''}</option>`).join('');
    if (seciliNo) noSel.value = seciliNo;
}

function daireDuzenleKgFiyatGuncelle() {
    const ton = parseFloat(document.getElementById('dDuzTonFiyat')?.value) || 0;
    const para = document.getElementById('dDuzPara')?.value || 'TRY';
    const el = document.getElementById('dDuzKgFiyat');
    if (!el) return;
    if (!ton) { el.value = ''; return; }
    const kg = ton / 1000;
    el.value = `${kg.toFixed(4)} ${para === 'USD' ? 'USD' : '₺'}/kg`;
}

function topluAnlasmaTahminGuncelle() {
    const blok = document.getElementById('taAktifBlok')?.value;
    const ton = parseFloat(document.getElementById('taToplamTon')?.value) || 0;
    const tonF = parseFloat(document.getElementById('taTonFiyat')?.value) || 0;
    const para = document.getElementById('taPara')?.value || 'TRY';
    const daireler = aptBlokFiltreliDaireler(window.aktifApartmanDaireler, blok);
    const adet = daireler.length || 0;
    const kgD = adet && ton ? (ton * 1000) / adet : 0;
    const kgF = tonF ? tonF / 1000 : 0;
    const kgEl = document.getElementById('taKgDaire');
    const fEl = document.getElementById('taKgFiyat');
    const oz = document.getElementById('taBlokOzet');
    if (kgEl) kgEl.value = adet ? `${aptSayi(kgD)} kg × ${adet} daire` : '—';
    if (fEl) fEl.value = tonF ? `${kgF.toFixed(4)} ${para}/kg` : '—';
    if (oz) oz.textContent = adet ? `${blok || '—'} bloğunda ${adet} daire · toplam ${aptSayi(ton * 1000)} kg` : 'Bu blokta daire yok';
}

async function aptListeleriYukle() {
    try {
        if (!aptMusteriCache.length) {
            const mRes = await fetch('/api/musteriler');
            aptMusteriCache = await mRes.json();
        }
        if (!aptUrunCache.length) {
            const uRes = await fetch('/api/komur');
            aptUrunCache = await uRes.json();
        }
    } catch (e) { console.error('apt liste yükleme:', e); }
}
...
window.apartmanlariYukle = async function() {
    ...
    try {
        const res = await fetch('/api/apartmanlar?_t=' + Date.now());
        const rows = await res.json();
        if (!res.ok) throw new Error(rows.hata || 'Liste alınamadı');
        window.aptCache = Array.isArray(rows) ? rows : [];
        apartmanKartlariCiz(window.aptCache);
    } catch (e) {
        kutu.innerHTML = `<div class="text-center text-danger py-4">${aptKacis(e.message)}</div>`;
    }
};

function apartmanKartlariCiz(liste) {
    const kutu = document.getElementById('apartmanKartListesi');
    if (!liste.length) {
        kutu.innerHTML = '<div class="text-center text-muted py-4">Henüz apartman yok. "Yeni Apartman" ile ekleyin.</div>';
        return;
    }
...
window.apartmanFiltrele = function() {
    const q = (document.getElementById('apartmanAra').value || '').toLocaleLowerCase('tr-TR').trim();
    if (!q) return apartmanKartlariCiz(window.aptCache);
    apartmanKartlariCiz(window.aptCache.filter((a) => {
        const blob = [a.Ad, a.Mahalle, a.Ilce, a.SorumluAd].filter(Boolean).join(' ').toLocaleLowerCase('tr-TR');
        return blob.includes(q);
    }));
};

window.apartmanEkleModalAc = function(duzenle) {
    document.getElementById('apartmanDuzenleId').value = duzenle ? (window.aktifApartman?.Id || '') : '';
...
window.apartmanKaydet = async function() {
    ...
    const govde = {
        ...
        aciklama: document.getElementById('apartmanAciklama').value.trim()
    };
    if (!govde.ad) { alert('Apartman adı gerekli'); return; }
    try {
        const url = id ? `/api/apartman/${id}` : '/api/apartman';
        const res = await fetch(url, {
            method: id ? 'PUT' : 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function apartmanDetayCiz() {
    ...
    const toplamOdenenKg = daireler.reduce((t, d) => t + aptOdenenKg(d), 0);
    const toplamBorcKg = Math.max(0, Math.round((toplamAnlasilan - toplamOdenenKg) * 100) / 100);
    ...
    document.getElementById('apartmanDetayOzet').innerHTML = `
        ...
        <div class="col-6 col-md-3"><div class="card bg-light border-0"><div class="card-body py-2 text-center"><div class="small text-muted">Müşteri Bağlı</div><div class="fw-bold fs-5">${dolu}</div></div></div></div>
        <div class="col-6 col-md-3"><div class="card bg-light border-0"><div class="card-body py-2 text-center"><div class="small text-muted">Teslim / Anlaş. kg</div><div class="fw-bold fs-6">${aptSayi(toplamTeslim)} / ${aptSayi(toplamAnlasilan)}</div></div></div></div>
        <div class="col-6 col-md-3"><div class="card bg-light border-0"><div class="card-body py-2 text-center"><div class="small text-muted">Ödenen / Borç kg</div><div class="fw-bold fs-6"><span class="text-success">${aptSayi(toplamOdenenKg)}</span> / <span class="text-danger">${aptSayi(toplamBorcKg)}</span></div></div></div></div>
    `;
    ...
    tbody.innerHTML = filtreli.map((d) => {
        const anlasilan = parseFloat(d.AnlasilanMiktar) || 0;
        const odenenKg = aptOdenenKg(d);
        const borcKg = aptBorcKg(d);
        let durum, durumCls;
        if (anlasilan > 0 && borcKg <= 0.01) { durum = 'Ödendi'; durumCls = 'bg-success'; }
        else if (odenenKg > 0) { durum = 'Kısmi ödeme'; durumCls = 'bg-warning text-dark'; }
        ...
        return `<tr role="button" onclick="daireDuzenleAc(${d.Id})">
            ...
            <td class="text-end ${borcKg > 0 ? 'text-danger fw-bold' : 'text-success'}">${anlasilan ? aptSayi(borcKg) : '-'}</td>
            <td class="text-center"><span class="badge ${durumCls}">${durum}</span></td>
        </tr>`;
    }).join('');
}
...
function aptBlokTeslimatPaneliCiz(filtreli) {
    ...
    const teslimBtn = (blok && blok !== 'TUMU')
        ? `<button class="btn btn-success btn-sm fw-bold" onclick="blokTeslimatAc('${aptKacis(blok).replace(/'/g, "\\'")}')"><i class="fas fa-truck-ramp-box me-1"></i> Teslimat Gir</button>`
        : `<span class="text-muted small">Teslimat girmek için bir blok sekmesi seçin</span>`;
    panel.innerHTML = `
        <div class="card border-success border-opacity-25 bg-light">
            <div class="card-body py-2">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
                    <div>
                        <span class="fw-bold text-dark"><i class="fas fa-truck text-success me-1"></i> ${aptKacis(blokAd)} Teslimat Durumu</span>
                        <div class="small mt-1">
                            Anlaşılan: <b>${aptSayi(toplamAnlasKg / 1000)} ton</b> ·
                            <span class="text-success">İnen: <b>${aptSayi(toplamTeslimKg / 1000)} ton</b></span> ·
...
window.aptBlokSec = function(blok) {
    window.aktifAptBlok = blok;
    apartmanDetayCiz();
};
...
window.daireOlustur = async function() {
    const blok = document.getElementById('daireBlok').value.trim();
    const baslangic = parseInt(document.getElementById('daireBaslangic').value, 10);
    const bitis = parseInt(document.getElementById('daireBitis').value, 10);
    if (!Number.isFinite(baslangic) || !Number.isFinite(bitis) || bitis < baslangic) {
        alert('Geçerli daire aralığı girin'); return;
    }
    try {
        const res = await fetch(`/api/apartman/${window.aktifApartman.Id}/daireler`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function aptMusteriSecenekleri(seciliKimlik) {
    const secenekler = aptMusteriCache.map((m) => {
        const ad = m.Unvan || m.Adı || ('#' + m.Kimlik);
        const sel = String(m.Kimlik) === String(seciliKimlik) ? 'selected' : '';
        return `<option value="${m.Kimlik}" ${sel}>${aptKacis(ad)}${m.CEPTEL && m.CEPTEL !== '-' ? ' · ' + aptKacis(m.CEPTEL) : ''}</option>`;
    }).join('');
    return '<option value="">— Seçilmedi —</option>' + secenekler;
}

function aptUrunSecenekleri(seciliId) {
    const secenekler = aptUrunCache.map((u) => {
        const sel = String(u.id) === String(seciliId) ? 'selected' : '';
        return `<option value="${u.id}" data-birim="${aptKacis(u.birim_turu || '')}" data-fiyat="${u.ton_fiyati || ''}" ${sel}>${aptKacis(u.cins || u.temel_ad)}</option>`;
    }).join('');
    return '<option value="">— Ürün —</option>' + secenekler;
}

window.daireDuzenleAc = async function(daireId) {
    await aptListeleriYukle();
...
document.addEventListener('change', function(e) {
    if (e.target && e.target.id === 'dDuzUrun') {
        const opt = e.target.options[e.target.selectedIndex];
        if (opt && opt.value) {
            if (!document.getElementById('dDuzTonFiyat').value) document.getElementById('dDuzTonFiyat').value = opt.getAttribute('data-fiyat') || '';
            daireDuzenleKgFiyatGuncelle();
        }
    }
    if (e.target && e.target.id === 'taUrun') {
        const opt = e.target.options[e.target.selectedIndex];
        if (opt && opt.value && !document.getElementById('taTonFiyat').value) {
            document.getElementById('taTonFiyat').value = opt.getAttribute('data-fiyat') || '';
            topluAnlasmaTahminGuncelle();
        }
    }
    if (e.target && (e.target.id === 'dDuzBlokSec' || e.target.id === 'dDuzNoSec')) {
        const blok = document.getElementById('dDuzBlokSec').value;
        document.getElementById('dDuzBlok').value = blok === '(Bloksuz)' ? '' : blok;
        document.getElementById('dDuzNo').value = document.getElementById('dDuzNoSec').value;
        const no = document.getElementById('dDuzNoSec').value;
        const bul = (window.aktifApartmanDaireler || []).find((x) => {
            const b = (x.Blok && String(x.Blok).trim()) || '(Bloksuz)';
            return b === blok && String(x.DaireNo || '') === String(no);
        });
        if (bul && bul.Id !== parseInt(document.getElementById('daireDuzenleId').value, 10)) {
            daireDuzenleAc(bul.Id);
        }
    }
    if (e.target && (e.target.id === 'dDuzTonFiyat' || e.target.id === 'dDuzPara')) daireDuzenleKgFiyatGuncelle();
});
document.addEventListener('input', function(e) {
    if (e.target && (e.target.id === 'taToplamTon' || e.target.id === 'taTonFiyat' || e.target.id === 'taPara')) topluAnlasmaTahminGuncelle();
    if (e.target && (e.target.id === 'dDuzTonFiyat' || e.target.id === 'dDuzPara')) daireDuzenleKgFiyatGuncelle();
});
...
window.daireKaydet = async function() {
    ...
    const govde = {
        ...
        birim: 'Kg',
        birimFiyat: tonF ? tonF / 1000 : 0,
        tonFiyat: tonF || null,
        paraBirimi: document.getElementById('dDuzPara').value || 'TRY',
        kalanBorcKg: document.getElementById('dDuzKalanBorcKg').value,
        daireTipi: document.getElementById('dDuzTip').value,
        aciklama: document.getElementById('dDuzAciklama').value.trim()
    };
    try {
        const res = await fetch(`/api/apartman-daire/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(govde)
        });
...
window.daireSil = async function() {
    const id = document.getElementById('daireDuzenleId').value;
    if (!id) return;
    if (!confirm('Bu daireyi silmek istediğinize emin misiniz? (Teslimat kayıtları da silinir)')) return;
    try {
...
// --- Blok bazlı teslimat ---
window.blokTeslimatAc = function(blok) {
    if (!blok || blok === 'TUMU') { alert('Lütfen önce bir blok sekmesi seçin.'); return; }
    document.getElementById('btBlok').value = blok;
    document.getElementById('btApartmanId').value = window.aktifApartman?.Id || '';
    document.getElementById('btMiktar').value = '';
    document.getElementById('btBirim').value = 'ton';
...
function btBilgiTazele() {
    const blok = document.getElementById('btBlok').value;
    const daireler = aptBlokFiltreliDaireler(window.aktifApartmanDaireler, blok)
        .filter((d) => d.UrunID && (parseFloat(d.AnlasilanMiktar) || 0) > 0);
    if (!daireler.length) { document.getElementById('btBilgi').innerHTML = 'Bu blokta anlaşmalı daire yok.'; return; }
    const toplamAnlas = daireler.reduce((t, d) => t + (parseFloat(d.AnlasilanMiktar) || 0), 0);
    const toplamTeslim = daireler.reduce((t, d) => t + (parseFloat(d.TeslimEdilen) || 0), 0);
    const kalan = Math.max(0, toplamAnlas - toplamTeslim);
    document.getElementById('btBilgi').innerHTML =
        `<b>${aptKacis(window.aktifApartman?.Ad || '')} · ${aptKacis(blok)} Blok</b> (${daireler.length} daire)<br>
         Anlaşılan: <b>${aptSayi(toplamAnlas / 1000)} ton</b> · İnen: <b class="text-success">${aptSayi(toplamTeslim / 1000)} ton</b> · Kalan: <b class="text-danger">${aptSayi(kalan / 1000)} ton</b>`;
    document.getElementById('btKgKarsilik').textContent = `Kalan teslimat: ${aptSayi(kalan)} kg`;
}

async function btGecmisYukle() {
    const kutu = document.getElementById('btGecmisListe');
    const blok = document.getElementById('btBlok').value;
    const aptId = document.getElementById('btApartmanId').value;
    if (!kutu || !aptId) return;
    kutu.innerHTML = '<div class="text-muted small text-center py-4">Yükleniyor…</div>';
    try {
        const res = await fetch(`/api/apartman/${aptId}/blok-teslimat-gecmisi?blok=${encodeURIComponent(blok)}&_t=` + Date.now());
        const rows = await res.json();
        if (!Array.isArray(rows) || !rows.length) {
            kutu.innerHTML = '<div class="text-muted small text-center py-4">Henüz teslimat girilmemiş.</div>';
            document.getElementById('btGecmisToplam').textContent = '0 kg';
            return;
        }
        let toplam = 0;
        kutu.innerHTML = rows.map((r) => {
            const kg = parseFloat(r.ToplamKg) || 0;
            toplam += kg;
            const tar = r.Tarih ? String(r.Tarih).slice(0, 10).split('-').reverse().join('.') : '';
            return `<div class="d-flex justify-content-between align-items-center border rounded px-2 py-2 mb-2 bg-white">
                <div>
                    <div class="fw-bold text-success">${aptSayi(kg)} kg <span class="text-muted small">(${aptSayi(kg/1000)} ton)</span></div>
                    <div class="small text-muted">${tar} · ${r.DaireSayisi} daire · ${aptKacis(r.IslemiYapan || '')}</div>
                </div>
                <button class="btn btn-outline-danger btn-sm" title="Geri al" onclick="btTeslimatSil('${aptKacis(r.Grup)}')"><i class="fas fa-trash"></i></button>
            </div>`;
        }).join('');
        document.getElementById('btGecmisToplam').textContent = `${aptSayi(toplam)} kg`;
    } catch (e) {
        kutu.innerHTML = '<div class="text-danger small text-center py-4">Geçmiş yüklenemedi.</div>';
    }
}

window.btTeslimatSil = async function(grup) {
    if (!grup) return;
    if (!confirm('Bu teslimatı geri almak istediğinize emin misiniz? Stok ve inen miktar düzeltilecek.')) return;
    try {
        const res = await fetch(`/api/apartman/blok-teslimat-grup/${encodeURIComponent(grup)}`, { method: 'DELETE' });
        const data = await res.json();
        if (!res.ok) throw new Error(data.hata || 'İşlem başarısız');
        await btVeriTazele();
    } catch (e) { alert('Hata: ' + e.message); }
};

async function btVeriTazele() {
    // Aktif apartman dairelerini tazele, panelleri güncelle, modal içi bilgi + geçmişi yenile
    const aptId = document.getElementById('btApartmanId').value;
    if (aptId) {
        try {
            const r = await fetch(`/api/apartman/${aptId}?_t=` + Date.now());
            const d = await r.json();
            if (r.ok) {
                if (d.apartman) window.aktifApartman = d.apartman;
                window.aktifApartmanDaireler = d.daireler || [];
            }
        } catch (_) {}
    }
    if (typeof apartmanDetayCiz === 'function') apartmanDetayCiz();
...
window.btBirimGuncelle = function() {
    const birim = document.getElementById('btBirim').value;
    const v = parseFloat(document.getElementById('btMiktar').value) || 0;
    const kg = birim === 'ton' ? v * 1000 : v;
    document.getElementById('btKgKarsilik').textContent = v > 0 ? `= ${aptSayi(kg)} kg dağıtılacak` : '';
};

window.blokTeslimatKaydet = async function() {
    const blok = document.getElementById('btBlok').value;
    const birim = document.getElementById('btBirim').value;
    const v = parseFloat(document.getElementById('btMiktar').value);
    if (!v || v <= 0) { alert('Geçerli miktar girin'); return; }
    const miktarKg = birim === 'ton' ? v * 1000 : v;
    const tarihStr = document.getElementById('btTarih').value;
    const tarih = tarihStr ? new Date(tarihStr).toISOString() : new Date().toISOString();
    const islemiYapan = localStorage.getItem('aktifKullanici') || 'Sistem';
    try {
        let res = await fetch(`/api/apartman/${window.aktifApartman.Id}/blok-teslimat`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ blok, miktar: miktarKg, tarih, islemiYapan })
        });
        let data = await res.json();
        if (res.status === 400 && data.kod === 'ASIM') {
            if (!confirm(`${data.hata}\n\nYine de teslim edilsin mi?`)) return;
            res = await fetch(`/api/apartman/${window.aktifApartman.Id}/blok-teslimat`, {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ blok, miktar: miktarKg, tarih, islemiYapan, izinliAsim: true })
            });
            data = await res.json();
        }
        if (!res.ok) throw new Error(data.hata || 'Teslimat başarısız');
        document.getElementById('btMiktar').value = '';
        document.getElementById('btKgKarsilik').textContent = '';
        await btVeriTazele();
    } catch (e) { alert('Hata: ' + e.message); }
};

window.daireTeslimatAc = async function(daireId) {
    const d = window.aktifApartmanDaireler.find((x) => x.Id === daireId);
    if (!d) return;
    const anlasilan = parseFloat(d.AnlasilanMiktar) || 0;
    const teslim = parseFloat(d.TeslimEdilen) || 0;
    const kalan = anlasilan - teslim;
    document.getElementById('teslimatDaireId').value = d.Id;
    document.getElementById('teslimatDaireBilgi').innerHTML =
        `<strong>${aptKacis(window.aktifApartman?.Ad || '')} · ${aptKacis(d.Blok || '')} Blok Daire ${aptKacis(d.DaireNo || '')}${d.DaireTipi ? ' (' + aptKacis(d.DaireTipi) + ')' : ''}</strong><br>
         ${aptKacis(d.MusteriAd || '')}<br>
         Ürün: ${aptKacis(d.UrunAdi || '-')} · ${d.TonFiyat ? aptSayi(d.TonFiyat) + ' ' + (d.ParaBirimi || 'TRY') + '/ton' : aptPara(d.BirimFiyat) + '/kg'}`;
...
async function daireTeslimatGecmisiYukle(daireId) {
    try {
        const res = await fetch(`/api/apartman-daire/${daireId}/teslimatlar?_t=` + Date.now());
        const rows = await res.json();
        const kutu = document.getElementById('teslimatGecmisi');
        if (!Array.isArray(rows) || !rows.length) { kutu.innerHTML = ''; return; }
        kutu.innerHTML = '<div class="small fw-bold text-muted mb-1">Teslimat geçmişi</div>' +
            rows.map((t) => {
                const tar = (t.Tarih || '').split(' ')[0];
                return `<div class="d-flex justify-content-between border-bottom py-1 small">
                    <span>${aptKacis(tar)} · ${aptSayi(t.Miktar)}</span>
                    <button class="btn btn-sm btn-link text-danger p-0" onclick="daireTeslimatGeriAl(${t.Id})">geri al</button>
                </div>`;
            }).join('');
    } catch (_) {}
}

window.daireTeslimatGeriAl = async function(teslimatId) {
    if (!confirm('Bu teslimatı geri almak istediğinize emin misiniz? (Cari borç ve stok düzeltilir)')) return;
    try {
...
window.daireTeslimatKaydet = async function() {
    const id = document.getElementById('teslimatDaireId').value;
    const miktar = parseFloat(document.getElementById('teslimatMiktar').value);
    if (!miktar || miktar <= 0) { alert('Geçerli miktar girin'); return; }
    try {
        let res = await fetch(`/api/apartman-daire/${id}/teslimat`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ miktar, tarih: new Date().toISOString() })
        });
        let data = await res.json();
        if (res.status === 400 && data.kod === 'ASIM') {
            if (!confirm(`${data.hata}\n\nYine de teslim edilsin mi?`)) return;
            res = await fetch(`/api/apartman-daire/${id}/teslimat`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ miktar, tarih: new Date().toISOString(), izinliAsim: true })
            });
            data = await res.json();
        }
...
window.topluAnlasmaModalAc = async function() {
    if (!window.aktifApartman) return;
    await aptListeleriYukle();
...
function taBlokOdemeVar(blok) {
    const daireler = aptBlokFiltreliDaireler(window.aktifApartmanDaireler, blok);
    return daireler.some((d) => {
        const anl = parseFloat(d.AnlasilanMiktar) || 0;
        const odenenKg = typeof aptOdenenKg === 'function'
            ? aptOdenenKg(d)
            : Math.max(0, anl - (d.KalanBorcKg != null ? parseFloat(d.KalanBorcKg) : anl));
        const odTl = parseFloat(d.OdenenTl) || 0;
        return odTl > 0.01 || odenenKg > 0.01;
    });
}

function taBlokBosDaireVar(blok) {
    return aptBlokFiltreliDaireler(window.aktifApartmanDaireler, blok)
        .some((d) => (parseFloat(d.AnlasilanMiktar) || 0) <= 0);
}

function taFormKilit(kilitli, opts) {
    const odemeKilit = !!(opts && opts.odemeKilit);
    window.taKilitli = !!kilitli && !odemeKilit;
    window.taOdemeKilit = odemeKilit;
    ['taUrun', 'taToplamTon', 'taPara', 'taTonFiyat', 'taDaireTipi'].forEach((id) => {
        const el = document.getElementById(id);
        // Ödeme kilidinde boş daireye yazmak için form açık kalır
        if (el) el.disabled = !!kilitli && !odemeKilit;
    });
    const sadeceBos = document.getElementById('taSadeceBos');
    if (sadeceBos) {
        if (odemeKilit) {
            sadeceBos.checked = true;
            sadeceBos.disabled = true;
        } else {
            sadeceBos.disabled = !!kilitli;
        }
    }
    const btn = document.getElementById('taUygulaBtn');
    const blok = document.getElementById('taAktifBlok')?.value;
    const tamamenPasif = odemeKilit && blok && !taBlokBosDaireVar(blok);
    if (btn) {
        const disabled = tamamenPasif || (!!kilitli && !odemeKilit);
        btn.disabled = disabled;
        btn.classList.toggle('disabled', disabled);
    }
    const kutu = document.getElementById('taKilitKutu');
    if (kutu) kutu.classList.toggle('d-none', !kilitli || odemeKilit);
    const odemeKutu = document.getElementById('taOdemeKilitKutu');
    if (odemeKutu) odemeKutu.classList.toggle('d-none', !odemeKilit);
    const unlockBtn = document.getElementById('taKilidiAcBtn');
    if (unlockBtn) unlockBtn.classList.toggle('d-none', !!odemeKilit);
}

window.taKilidiAc = function() {
    const blok = document.getElementById('taAktifBlok')?.value;
    if (blok && taBlokOdemeVar(blok)) {
        alert('Bu blokta ödeme hareketi var. Tekrar anlaşma açılamaz.');
        return;
    }
...
function taBlokFormunuDoldur(blok) {
    document.getElementById('taUrun').innerHTML = aptUrunSecenekleri('');
    const odemeVar = taBlokOdemeVar(blok);
    ...
    if (mevcut) {
        ...
        document.getElementById('taSadeceBos').checked = !!odemeVar;
        taFormKilit(true, { odemeKilit: odemeVar });
        ...
        if (oz) {
            oz.innerHTML = `<span class="text-success fw-bold">Mevcut anlaşma:</span> ${aptSayi(mevcut.toplamTon)} ton · ${aptSayi(mevcut.tonFiyat)} ${mevcut.paraBirimi}/ton · ${mevcut.anlasmaliSayi} daire · ${aptKacis(mevcut.urunAdi || '')}`
                + (odemeVar ? ' · <span class="text-danger fw-bold">Ödeme var — tekrar anlaşma kapalı</span>' : '');
        }
    } else {
        ...
        taFormKilit(false, { odemeKilit: odemeVar });
        const yeni = document.getElementById('taYeniBilgi');
        if (yeni) yeni.classList.toggle('d-none', odemeVar);
        const kutu = document.getElementById('taKilitKutu');
        if (kutu) kutu.classList.add('d-none');
        const oz = document.getElementById('taBlokOzet');
        if (oz) {
            oz.innerHTML = odemeVar
                ? '<span class="text-danger fw-bold">Bu blokta ödeme var — yalnızca boş dairelere anlaşma yazılabilir.</span>'
                : '';
        }
    }
    topluAnlasmaTahminGuncelle();
...
window.blokAnlasmaUygula = async function() {
    ...
    const odemeVar = taBlokOdemeVar(blok);
    if (odemeVar) {
        // Ödeme olan blokta yalnızca boş dairelere izin
        document.getElementById('taSadeceBos').checked = true;
    }
    const govde = {
        blok: blok === '(Bloksuz)' ? '' : blok,
        urunId: document.getElementById('taUrun').value || null,
        toplamTon: parseFloat(document.getElementById('taToplamTon').value) || 0,
        tonFiyat: parseFloat(document.getElementById('taTonFiyat').value) || 0,
        paraBirimi: document.getElementById('taPara').value || 'TRY',
        daireTipi: document.getElementById('taDaireTipi').value || null,
        sadeceBos: document.getElementById('taSadeceBos').checked || odemeVar
    };
    if (odemeVar && !govde.sadeceBos) {
        alert('Bu blokta ödeme hareketi var. Tekrar anlaşma uygulanamaz.');
        return;
    }
    if (odemeVar && !taBlokBosDaireVar(blok)) {
        alert('Bu blokta ödeme var ve boş daire kalmamış. Tekrar anlaşma yapılamaz.');
        return;
    }
    if (!govde.urunId || govde.toplamTon <= 0 || govde.tonFiyat <= 0) {
...
window.musteriApartmanBilgisiYukle = async function(musteriId) {
    ...
    try {
        ...
        liste.innerHTML = rows.map((d) => {
            ...
            const yer = `${d.Blok ? d.Blok + ' Blok · ' : ''}Daire ${d.DaireNo || ''}${d.DaireTipi ? ' (' + d.DaireTipi + ')' : ''}`;
            const borcKg = d.KalanBorcKg != null ? parseFloat(d.KalanBorcKg) : anlasilan;
            return `<div class="col-md-6">
                <div class="border rounded-3 p-2 h-100 d-flex justify-content-between align-items-center" role="button" onclick="apartmanDetayAcMusteriden(${d.ApartmanId})">
                    <div>
                        ...
                        <div class="small">Anlaş. ${aptSayi(anlasilan)} kg · Teslim ${aptSayi(teslim)} kg · <span class="${kalan > 0 ? 'text-danger' : 'text-success'} fw-bold">Kalan ${aptSayi(kalan)} kg</span>
                        · Borç ${aptSayi(borcKg)} kg${d.TonFiyat ? ' · ' + aptSayi(d.TonFiyat) + ' ' + (d.ParaBirimi || 'TRY') + '/ton' : ''}</div>
                    </div>
...
window.topluAnlasmaUygula = window.blokAnlasmaUygula;